...
IMPORTANT: Use the exact price $${rd?.price||"unknown"} in your response. Do not invent a different price.`,
      );
      // Override AI price with verified real price
      if(rd?.price) r.price = `$${rd.price}`;
      if(rd?.quantScore) r.quantScore = rd.quantScore;
      if(rd?.quantGrade) r.quantGrade = rd.quantGrade;
...
IMPORTANT: Use the exact price $${rd?.price||"unknown"}. Do not invent a different price.`);
      if(rd?.price) r.price = `$${rd.price}`;
      if(rd?.analystTarget) r.priceTarget = `$${rd.analystTarget}`;
      if(rd?.impliedUpside) r.upside = `+${rd.impliedUpside}%`;
      if(rd?.peRatio) r.peRatio = `${rd.peRatio}`;
...
Generate 3 options setups for ${selectedTicker} (${selectedStock.name}). Today: ${new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"})}. Stock price is EXACTLY $${rd?.price||"unknown"} -- use this for all strike calculations. Next earnings: ${rd?.nextEarnings||"search for it"}. Beta: ${rd?.beta||"N/A"}.`
      );
      if(rd?.price) r.stockPrice = `$${rd.price}`;
      setResult(r); setState("done");
    }catch{ setState("error"); }
  };
...
Generate 3 options setups for ${sym}. Today: ${new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"})}. Stock price is EXACTLY $${rd2?.price||"unknown"} -- use this for all strike calculations. Next earnings: ${rd2?.nextEarnings||"search for it"}.`
      );
      if(rd2?.price) r.stockPrice = `$${rd2.price}`;
...
IMPORTANT: Use the exact verified price $${rd?.price||"unknown"} in your response. The quant score for this stock is ${rd?.quantScore||"N/A"}/100. If stock does not meet ALL criteria set passesScreen to false.`
        );
        if(rd?.price) r.price = `$${rd.price}`;
        if(rd?.analystTarget) r.priceTarget = `$${rd.analystTarget}`;
        if(rd?.impliedUpside) r.upside = `+${rd.impliedUpside}%`;
        if(rd?.peRatio) r.peRatio = `${rd.peRatio}`;
        if(rd?.revenueGrowth) r.revenueGrowth = `+${rd.revenueGrowth}% YoY`;
        if(rd?.epsGrowth) r.earningsGrowth = `+${rd.epsGrowth}% YoY`;
        if(rd?.analystConsensus) r.analystConsensus = rd.analystConsensus;
...
IMPORTANT: The verified current price is $${rd?.price||lp?.price?.toFixed(2)||"unknown"}. Use this exact price.`
      );
      if(rd?.price) r.price = `$${rd.price}`;
      if(rd?.analystTarget) r.priceTarget = `$${rd.analystTarget}`;
      if(rd?.impliedUpside) r.upside = `+${rd.impliedUpside}%`;
  ...
  return(
    <div style={{minHeight:"100vh",background:BG}}>
      ...
      <div style={{borderTop:`1px solid ${BORDER}`,padding:"16px 28px",display:"flex",justifyContent:"center",gap:16,flexWrap:"wrap",alignItems:"center"}}>
        ...
        <span style={{color:"#1a2a3a",fontSize:11,fontFamily:"'DM Sans',sans-serif"}}>(c) 2026 StockMoolah. All rights reserved.</span>
      </div>
