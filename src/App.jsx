...
// ─── SYSTEM PROMPTS ──────────────────────────────────────────────────────────
const STOCK_SYSTEM = `You are a professional stock analyst with real-time market access. Search for the latest data on the requested stock. Return ONLY valid JSON (no markdown): { "ticker": "XXXX", "name": "Company Name", "verdict": "STRONG BUY"|"BUY"|"SPECULATIVE"|"HOLD"|"SELL"|"STRONG SELL", "confidence": 0-100, "price": "$XXX.XX", "priceTarget": "$XXX", "analystConsensus": "Buy/Hold/Sell", "upside": "+XX%", "eps": "Latest EPS vs estimate", "revenue": "Latest revenue vs estimate", "nextEarnings": "Month DD YYYY", "peRatio": "XX.X", "catalysts": ["catalyst 1","catalyst 2","catalyst 3"], "risks": ["risk 1","risk 2"], "recentNews": ["news 1","news 2","news 3"], "whyThisRating": "3-4 sentence explanation of the rating", "dataSources": ["source1","source2"] }`;

const GEM_SYSTEM = `You are a small/mid-cap specialist. Search for the latest data on this hidden gem stock. Return ONLY valid JSON (no markdown, no backticks): { "ticker": "XXXX", "name": "Company Name", "verdict": "STRONG BUY"|"BUY"|"SPECULATIVE"|"HOLD"|"SELL"|"STRONG SELL", "confidence": 0-100, "price": "$XXX.XX", "priceTarget": "$XXX", "analystConsensus": "Buy/Hold/Sell", "upside": "+XX%", "eps": "Latest EPS vs estimate", "revenue": "Latest revenue vs estimate", "nextEarnings": "Month DD YYYY", "peRatio": "XX.X", "catalysts": ["catalyst 1","catalyst 2","catalyst 3"], "risks": ["risk 1","risk 2"], "recentNews": ["news 1","news 2","news 3"], "whyThisRating": "3-4 sentence explanation of why this is a hidden gem worth investing in", "dataSources": ["source1","source2"] }`;

const IPO_SYSTEM = `You are an IPO analyst. Search for the latest data on this upcoming IPO. Return ONLY valid JSON: { "verdict": "STRONG BUY"|"BUY"|"SPECULATIVE"|"HOLD"|"AVOID", "confidence": 0-100, "valuation": "$XXB", "priceRange": "$XX-$XX", "whyThisRating": "3-4 sentence explanation", "catalysts": ["catalyst 1","catalyst 2"], "risks": ["risk 1","risk 2"], "comparable": "Comparable public company", "dataSources": ["source1"] }`;

const OPTIONS_SYSTEM = `You are an elite options trader. Search for live data on this stock and generate 3 options setups. Return ONLY valid JSON: { "ticker": "XXXX", "stockPrice": "$XXX", "marketBias": "BULLISH"|"BEARISH"|"NEUTRAL"|"VOLATILE", "ivRank": "XX%", "nextEarnings": "Month DD YYYY", "trades": [ { "type": "CALL"|"PUT"|"SPREAD"|"CONDOR", "riskLevel": "CONSERVATIVE"|"MODERATE"|"AGGRESSIVE", "strategy": "Strategy name", "strike": "$XXX", "expiry": "Month DD YYYY", "premium": "$X.XX", "maxProfit": "+XX%", "maxLoss": "-XX%", "breakeven": "$XXX", "rationale": "Why this trade makes sense", "catalyst": "What could drive the move" } ], "optionsDisclaimer": "Brief risk warning" }`;

const SMART_MONEY_SYSTEM = `You are a financial intelligence analyst. Search for the most recent disclosed trades for this investor. Return ONLY valid JSON: { "traderName": "Full Name", "lastUpdated": "Month DD YYYY", "recentTrades": [ { "ticker": "XXXX", "action": "BUY"|"SELL", "amount": "$XX,000-$XX,000", "date": "Month DD YYYY", "notes": "Context about this trade" } ], "currentTopHoldings": ["TICKER1","TICKER2","TICKER3"], "recentThesis": "What their recent trades suggest about their market outlook", "followSignal": "STRONG BUY"|"BUY"|"WATCH"|"NEUTRAL", "followConfidence": 0-100, "dataSource": "Source name" }`;

// ── SCREENER SYSTEM PROMPT ────────────────────────────────────────────────────
const SCREENER_SYSTEM = `You are a quantitative analyst running a stock screen. Analyze the following stock against the user's criteria and return a concise verdict. Return ONLY valid JSON (no markdown): { "ticker": "XXXX", "name": "Full Company Name", "sector": "Sector", "verdict": "STRONG BUY"|"BUY"|"SPECULATIVE"|"HOLD"|"SELL"|"STRONG SELL", "confidence": 0-100, "price": "$XXX.XX", "priceTarget": "$XXX", "upside": "+XX%", "peRatio": "XX.X", "revenueGrowth": "+XX% YoY", "earningsGrowth": "+XX% YoY", "analystConsensus": "Buy/Hold/Sell", "momentum": "STRONG"|"MODERATE"|"WEAK"|"NEGATIVE", "technicalRating": "BULLISH"|"NEUTRAL"|"BEARISH", "catalysts": ["catalyst 1","catalyst 2"], "whyThisRating": "2-3 sentence explanation", "passesScreen": true }`;

const DYNAMIC_WATCHLIST_SYSTEM = `You are a senior portfolio manager at a top-tier hedge fund with real-time market access. Your job is to curate the highest-conviction, highest-profit-potential stock picks for this week. Search for current market data. Prioritize: stocks with strong earnings momentum, sector leaders with institutional buying, upcoming catalysts (earnings, FDA decisions, product launches, analyst upgrades), and technically strong setups at key breakout levels. Eliminate any stock that is speculative without a clear near-term catalyst. Return ONLY valid JSON (no markdown): { "generatedAt": "Month DD YYYY HH:MM ET", "weekOf": "Week of Month DD YYYY", "marketContext": "2 sentences on the market environment this week and what is driving opportunities", "marketBias": "BULLISH"|"BEARISH"|"MIXED"|"VOLATILE", "editorNote": "1 sentence on your highest conviction theme this week", "stocks": [ { "ticker": "XXXX", "name": "Full Company Name", "sector": "Sector", "tag": "MOMENTUM"|"VALUE"|"GROWTH"|"EARNINGS"|"BREAKOUT"|"CATALYST"|"INSTITUTIONAL", "price": "$XXX.XX", "changeToday": "+/-X.X%", "weeklyChange": "+/-X.X%", "whyThisWeek": "Why this stock has high profit potential THIS WEEK specifically — be precise", "catalyst": "Specific upcoming catalyst with date if known", "technicalSetup": "Key technical level or pattern", "confidence": 0-100, "verdict": "STRONG BUY"|"BUY"|"SPECULATIVE", "target": "$XXX", "stopLoss": "$XXX", "timeframe": "Days or weeks to target" } ] }`;

const TRENDING_SYSTEM = `You are a market intelligence desk analyst at a premier trading firm. Your role is to surface the highest-profit-potential stocks with active momentum RIGHT NOW — not just what is trending socially, but what has real institutional sponsorship, unusual options flow, and a credible near-term catalyst. Only use REAL ticker symbols. Return ONLY valid JSON (no markdown): { "lastUpdated": "Month DD YYYY HH:MM ET", "marketMood": "BULLISH"|"BEARISH"|"MIXED"|"VOLATILE", "moodReason": "1 sentence on the dominant market force right now", "trending": [ { "ticker": "XXXX", "name": "Full Company Name", "sector": "Sector", "reason": "Precise reason this has high profit potential right now", "catalyst": "Specific catalyst — earnings date, news event, technical breakout level", "unusualFlow": true, "momentum": "STRONG"|"MODERATE"|"EARLY", "profitPotential": "HIGH"|"MEDIUM"|"SPECULATIVE", "entry": "$XXX", "target": "$XXX", "stopLoss": "$XXX", "confidence": 0-100, "verdict": "STRONG BUY"|"BUY"|"SPECULATIVE" } ], "topPickToday": "TICKER", "topPickReason": "2 sentences on why this is the single best opportunity right now" }`;

const DYNAMIC_SMART_MONEY_SYSTEM = `You are a financial intelligence analyst. Search for the most recent congressional STOCK Act trade disclosures and SEC 13F filings. ONLY use REAL names and REAL tickers. If no real recent data, return empty recentActivity array. Return ONLY valid JSON: { "lastUpdated": "Month DD YYYY HH:MM ET", "recentActivity": [ { "trader": "Full Name", "role": "Title", "category": "CONGRESS"|"HEDGE FUND", "trade": "Bought/Sold TICKER", "amount": "$XX,000-$XX,000", "date": "Month DD YYYY", "significance": "Why this matters", "followSignal": "STRONG BUY"|"BUY"|"WATCH"|"NEUTRAL", "confidence": 0-100 } ], "hotStocks": ["TICKER1","TICKER2","TICKER3"], "summary": "2 sentence summary of smart money activity right now" }`;

const DYNAMIC_IPO_SYSTEM = `You are a senior IPO analyst at an investment bank. Search for the most current IPO pipeline. Evaluate each on: valuation vs peers, revenue growth, path to profitability, lock-up expiry risk, and first-day pop potential. Only include companies with real filed S-1s or confirmed IPO dates. ONLY use REAL companies. Return ONLY valid JSON: { "lastUpdated": "Month DD YYYY", "marketNote": "1 sentence on current IPO market", "ipos": [ { "name": "Company Name", "ticker": "Expected ticker or TBD", "sector": "Sector", "description": "What the company does", "estTiming": "Q1 2026 or specific date", "valuation": "$XXB", "profitPotential": "HIGH"|"MEDIUM"|"SPECULATIVE", "buyConfidence": 0-100, "verdict": "STRONG BUY"|"BUY"|"WATCH"|"AVOID", "whyNow": "Why noteworthy right now", "risks": "Main risk" } ] }`;

const DYNAMIC_GEMS_SYSTEM = `You are a veteran small and mid-cap fund manager. Your edge is finding overlooked, undervalued stocks before institutions pile in. Search for stocks that are: trading below intrinsic value with an imminent re-rating catalyst, seeing insider buying or institutional accumulation, breaking out of long consolidation bases, or benefiting from a sector tailwind the market has not yet priced in. Minimum market cap $300M. ONLY use REAL tickers. Return ONLY valid JSON: { "lastUpdated": "Month DD YYYY", "weekOf": "Week of Month DD YYYY", "marketContext": "1-2 sentences on why this is a good environment for small/mid-cap gems right now", "gems": [ { "ticker": "XXXX", "name": "Full Company Name", "sector": "Sector", "marketCap": "$XB", "price": "$XXX.XX", "why": "Precise thesis — why this is undervalued and poised to move", "catalyst": "Specific upcoming catalyst with approximate timing", "insiderActivity": "Insider buying/selling activity if known", "upside": "+XX%", "priceTarget": "$XXX", "stopLoss": "$XXX", "timeframe": "Weeks or months", "confidence": 0-100, "verdict": "STRONG BUY"|"BUY"|"SPECULATIVE", "risk": "Main risk to the thesis" } ] }`;

const DYNAMIC_OPTIONS_SYSTEM = `You are the head options strategist at a top derivatives desk. Identify the highest-probability options trades available RIGHT NOW. Prioritize: stocks with earnings within 2 weeks (high IV crush potential), unusual call or put sweeps above $500K notional, stocks at key technical breakout levels, and sector leaders with defined catalysts. For each trade specify the exact contract. ONLY use REAL tickers. Return ONLY valid JSON: { "lastUpdated": "Month DD YYYY HH:MM ET", "marketVIX": "XX.X", "marketCondition": "LOW VOL"|"NORMAL"|"HIGH VOL"|"EXTREME", "vixTrend": "RISING"|"FALLING"|"STABLE", "topOpportunities": [ { "ticker": "XXXX", "name": "Company Name", "stockPrice": "$XXX.XX", "strategy": "Specific strategy name", "signal": "Precise reason this is the best options trade right now", "strike": "$XXX", "expiry": "Month DD YYYY", "type": "CALL"|"PUT"|"SPREAD"|"CONDOR", "riskLevel": "CONSERVATIVE"|"MODERATE"|"AGGRESSIVE", "estimatedPremium": "$X.XX", "maxProfit": "+XX%", "maxLoss": "-XX%", "breakeven": "$XXX", "confidence": 0-100, "catalyst": "Specific dated catalyst", "unusualActivity": true, "daysToExpiry": "XX days" } ], "unusualActivityAlerts": [ { "ticker": "XXXX", "activity": "Specific description — e.g. 5,000 calls swept at $X.XX", "bullish": true, "size": "$XXM", "expiry": "Month DD YYYY" } ], "earningsPlays": ["TICKER1 — Month DD","TICKER2 — Month DD"] }`;

const MOMENTUM_MOVERS_SYSTEM = `You are the head of a professional day trading desk. Your job is to identify the highest-conviction intraday trades available RIGHT NOW — stocks with genuine price momentum, real catalysts, clear technical setups, and favorable risk/reward ratios. Exclude low-float penny stocks. Prioritize liquid, exchange-listed stocks with institutional participation. ONLY use REAL ticker symbols. Return ONLY valid JSON: { "timestamp": "Month DD YYYY HH:MM ET", "marketSession": "PRE-MARKET"|"REGULAR"|"AFTER-HOURS"|"CLOSED", "marketMood": "RISK-ON"|"RISK-OFF"|"MIXED"|"VOLATILE", "topMovers": [ { "ticker": "XXXX", "name": "Company Name", "sector": "Sector", "price": "$XXX.XX", "change": "+X.XX", "changePct": "+XX.X%", "direction": "UP"|"DOWN", "volume": "XXM (Xx avg)", "catalyst": "Exact reason with specifics — earnings beat, upgrade, news event", "session": "REGULAR"|"AFTER-HOURS"|"PRE-MARKET", "momentum": "STRONG"|"BUILDING"|"FADING", "entryZone": "$XXX-$XXX", "target1": "$XXX", "target2": "$XXX", "stopLoss": "$XXX", "riskReward": "1:X", "confidence": 0-100, "signal": "STRONG BUY"|"BUY"|"WATCH"|"SHORT" } ], "marketHighlight": "Most important development driving the market right now", "bestDayTradeSetup": "TICKER — 1 sentence on why this is the top intraday setup" }`;

const SCALP_SETUPS_SYSTEM = `You are a professional scalp trader with a live feed to market data. Identify the cleanest, highest-probability scalp setups available RIGHT NOW. Focus only on liquid, high-volume stocks (avg volume >2M/day) with clear technical levels. Each setup must have a defined catalyst, precise entry trigger, and tight stop. ONLY use REAL tickers. If market is closed, return empty setups array. Return ONLY valid JSON: { "timestamp": "Month DD YYYY HH:MM ET", "session": "PRE-MARKET"|"REGULAR"|"AFTER-HOURS"|"CLOSED", "marketCondition": "Trending"|"Range-Bound"|"Volatile"|"Low Volume", "setups": [ { "ticker": "XXXX", "name": "Company Name", "currentPrice": "$XXX.XX", "avgVolume": "XXM/day", "setupType": "Breakout"|"Pullback"|"Gap and Go"|"Reversal"|"Squeeze Play"|"VWAP Reclaim", "timeframe": "1min"|"5min"|"15min", "entryTrigger": "Exact price action trigger — e.g. break above $XXX with volume", "entry": "$XXX.XX", "target1": "$XXX.XX", "target2": "$XXX.XX", "stopLoss": "$XXX.XX", "potentialGain": "+X.X%", "maxLoss": "-X.X%", "riskReward": "1:X", "catalyst": "Precise catalyst driving this setup", "urgency": "NOW"|"WATCH"|"PENDING", "confidence": 0-100, "notes": "Key invalidation level and what to watch for" } ], "avoidList": ["TICKER1","TICKER2"], "avoidReason": "Why to avoid these today" }`;

const SHORT_SQUEEZE_SYSTEM = `You are a short interest analyst specializing in identifying gamma and short squeezes before they peak. Search for stocks with: short float above 15%, days-to-cover above 3, recent unusual call buying, positive news catalyst, and price beginning to move against shorts. Rank by squeeze probability. ONLY use REAL tickers. Return ONLY valid JSON: { "timestamp": "Month DD YYYY HH:MM ET", "squeezeEnvironment": "FAVORABLE"|"NEUTRAL"|"UNFAVORABLE", "environmentNote": "1 sentence on current market conditions for squeezes", "candidates": [ { "ticker": "XXXX", "name": "Company Name", "currentPrice": "$XXX.XX", "shortFloat": "XX%", "shortInterest": "XX million shares", "daysToCover": "X.X days", "borrowRate": "XX% annually", "squeezeStage": "BUILDING"|"ACTIVE"|"IMMINENT"|"COOLING", "catalyst": "Specific catalyst that could force shorts to cover", "unusualCallActivity": true, "entryZone": "$XXX-$XXX", "squeezeTarget": "$XXX", "conservativeTarget": "$XXX", "stopLoss": "$XXX", "squeezeProbability": "HIGH"|"MEDIUM"|"LOW", "confidence": 0-100, "risk": "Main risk — e.g. dilution, weak fundamentals" } ], "activeSqueezes": ["TICKER1","TICKER2"], "watchForSqueeze": ["TICKER3","TICKER4"], "summary": "2 sentences on the current short squeeze landscape" }`;

const AFTERHOURS_SYSTEM = `You are an after-hours specialist. Search for stocks moving in extended hours RIGHT NOW. ONLY use REAL company names and REAL tickers verified against market data. Focus on moves with actionable next-day setups — earnings beats/misses, guidance raises/cuts, M&A, FDA decisions. Include specific trading plans with levels. Return ONLY valid JSON: { "timestamp": "Month DD YYYY HH:MM ET", "session": "AFTER-HOURS"|"PRE-MARKET"|"CLOSED", "bigMovers": [ { "ticker": "XXXX", "name": "Company Name", "regularClose": "$XXX.XX", "afterHoursPrice": "$XXX.XX", "afterHoursPct": "+/-XX.X%", "direction": "UP"|"DOWN", "catalyst": "Exact reason", "gapType": "EARNINGS_BEAT"|"EARNINGS_MISS"|"NEWS_POSITIVE"|"NEWS_NEGATIVE", "nextDayOutlook": "BULLISH"|"BEARISH"|"UNCERTAIN", "dayTradePlan": "How to trade tomorrow", "keyLevels": "Key price levels", "confidence": 0-100 } ], "earningsTonight": [ { "ticker": "XXXX", "name": "Company Name", "reportTime": "After Close"|"Before Open", "epsEstimate": "$X.XX", "revenueEstimate": "$XB", "impliedMove": "±X%" } ], "tomorrowWatchlist": ["TICKER1","TICKER2","TICKER3"], "summary": "2 sentences on key after-hours developments" }`;

const PREMARKET_SYSTEM = `You are the pre-market desk analyst at a trading firm. Search for pre-market movers, overnight futures, and economic events. Identify the most actionable gap plays and provide a concrete day-trading bias for each mover. ONLY use REAL tickers. Return ONLY valid JSON: { "timestamp": "Month DD YYYY HH:MM ET", "marketOutlook": "BULLISH OPEN"|"BEARISH OPEN"|"FLAT OPEN"|"VOLATILE OPEN", "futuresSnapshot": { "sp500": "+/-X.X%", "nasdaq": "+/-X.X%", "dow": "+/-X.X%", "vix": "XX.X" }, "preMarketMovers": [ { "ticker": "XXXX", "name": "Company Name", "preMarketPrice": "$XXX.XX", "preMarketPct": "+/-XX.X%", "catalyst": "What happened", "gapDirection": "UP"|"DOWN", "gapSize": "SMALL (<2%)"|"MEDIUM (2-5%)"|"LARGE (5-10%)"|"HUGE (10%+)", "tradeStrategy": "Gap and Go"|"Gap Fill"|"Wait and See", "keyOpenLevel": "$XXX", "firstTarget": "$XXX", "stopLoss": "$XXX", "confidence": 0-100 } ], "economicEvents": "Key events today", "dayTradingBias": "Overall bias for today" }`;
...
// ─── REAL DATA LAYER ─────────────────────────────────────────────────────────
// Priority: Finnhub → Alpha Vantage → Yahoo Finance → AI (narrative only, never prices)
// Set these in Netlify: VITE_FINNHUB_API_KEY and VITE_ALPHAVANTAGE_API_KEY
...
// ── In-memory cache for real data (5 min TTL) ────────────────────────────────
const _dataCache = {};
const _getCached = (key) => {
const e = _dataCache[key];
return e && Date.now()-e.ts < 5*60*1000 ? e.data : null;
};
const _setCache = (key, data) => { _dataCache[key] = {data, ts:Date.now()}; };

// ── Yahoo Finance fetch (primary free price source) ───────────────────────────
async function fetchYahooQuote(ticker) {
const cached = _getCached(`yq_${ticker}`);
...
const cached = _getCached(`fh_${endpoint}_${JSON.stringify(params)}`);
if(cached) return cached;
try{
const q = new URLSearchParams({…params, token: FINNHUB_KEY});
const res = await fetch(`${FINNHUB_BASE}${endpoint}?${q}`);
if(!res.ok) return null;
const data = await res.json();
_setCache(`fh_${endpoint}_${JSON.stringify(params)}`, data);
return data;
}catch(e){ return null; }
}

// Real-time quote from Finnhub (most accurate)
...
prev:      +data.pc.toFixed(2), // previous close
change:    +data.d.toFixed(2),  // change
changePct: +data.dp.toFixed(2), // change %
high:      +data.h.toFixed(2),  // day high
low:       +data.l.toFixed(2),  // day low
open:      +data.o.toFixed(2),  // open
source:    “finnhub”,
};
}
...
const m = data.metric;
return {
peRatio:         m[“peBasicExclExtraTTM”]      ? +m[“peBasicExclExtraTTM”].toFixed(2)  : null,
pbRatio:         m[“pbAnnual”]                  ? +m[“pbAnnual”].toFixed(2)             : null,
eps:             m[“epsBasicExclExtraItemsTTM”] ? +m[“epsBasicExclExtraItemsTTM”].toFixed(2) : null,
revenueGrowth:   m[“revenueGrowthTTMYoy”]      ? +m[“revenueGrowthTTMYoy”].toFixed(2)  : null,
epsGrowth:       m[“epsGrowthTTMYoy”]           ? +m[“epsGrowthTTMYoy”].toFixed(2)      : null,
roa:             m[“roaTTM”]                    ? +m[“roaTTM”].toFixed(2)               : null,
roe:             m[“roeTTM”]                    ? +m[“roeTTM”].toFixed(2)               : null,
debtEquity:      m[“totalDebt/totalEquityAnnual”] ? +m[“totalDebt/totalEquityAnnual”].toFixed(2) : null,
currentRatio:    m[“currentRatioAnnual”]        ? +m[“currentRatioAnnual”].toFixed(2)   : null,
grossMargin:     m[“grossMarginTTM”]            ? +m[“grossMarginTTM”].toFixed(2)       : null,
high52:          m[“52WeekHigh”]                ? +m[“52WeekHigh”].toFixed(2)           : null,
low52:           m[“52WeekLow”]                 ? +m[“52WeekLow”].toFixed(2)            : null,
...
const data = await finnhubGet(”/stock/recommendation”, {symbol: ticker});
...
const score   = total > 0 ? Math.round((bullish/total)*100) : null;
return {
...
total,
bullishPct:  score,
consensus:   score >= 60 ? “BUY” : score >= 40 ? “HOLD” : “SELL”,
...
const to    = new Date(now.getTime()+90*24*3600*1000).toISOString().split(“T”)[0];
const data  = await finnhubGet(”/calendar/earnings”, {symbol: ticker, from, to});
if(!data?.earningsCalendar?.length) return null;
const next  = data.earningsCalendar[0];
return {
date:        next.date,
epsEstimate: next.epsEstimate,
revenueEst:  next.revenueEstimate,
source:      “finnhub”,
};
}
...
mktCap:      data.marketCapitalization ? +(data.marketCapitalization*1e6).toFixed(0) : null,
...
// Alpha Vantage — income statement (used only when Finnhub key missing, 25 calls/day limit)
async function fetchAVIncome(ticker) {
if(!ALPHAVANTAGE_KEY) return null;
const cached = _getCached(`av_income_${ticker}`);
if(cached) return cached;
try{
const res  = await fetch(`${AV_BASE}?function=INCOME_STATEMENT&symbol=${ticker}&apikey=${ALPHAVANTAGE_KEY}`);
if(!res.ok) return null;
const data = await res.json();
if(data.Note || data.Information) return null; // rate limit hit
const annual = data.annualReports?.[0];
const prev   = data.annualReports?.[1];
if(!annual) return null;
const rev      = +annual.totalRevenue;
const prevRev  = prev ? +prev.totalRevenue : null;
const netInc   = +annual.netIncome;
const prevInc  = prev ? +prev.netIncome    : null;
const result   = {
revenue:       rev,
revenueGrowth: prevRev ? +((rev-prevRev)/Math.abs(prevRev)*100).toFixed(2) : null,
netIncome:     netInc,
netIncGrowth:  prevInc ? +((netInc-prevInc)/Math.abs(prevInc)*100).toFixed(2) : null,
grossProfit:   +annual.grossProfit,
ebitda:        +annual.ebitda,
fiscalYear:    annual.fiscalDateEnding,
source:        “alphavantage”,
};
_setCache(`av_income_${ticker}`, result);
return result;
}catch(e){ return null; }
}

// Alpha Vantage — overview (P/E, EPS, targets, sector — use sparingly)
async function fetchAVOverview(ticker) {
if(!ALPHAVANTAGE_KEY) return null;
const cached = _getCached(`av_overview_${ticker}`);
if(cached) return cached;
try{
const res  = await fetch(`${AV_BASE}?function=OVERVIEW&symbol=${ticker}&apikey=${ALPHAVANTAGE_KEY}`);
if(!res.ok) return null;
const d    = await res.json();
if(d.Note || d.Information || !d.Symbol) return null;
const result = {
name:           d.Name,
sector:         d.Sector,
industry:       d.Industry,
peRatio:        d.PERatio     !== “None” ? +d.PERatio     : null,
pbRatio:        d.PriceToBookRatio !== “None” ? +d.PriceToBookRatio : null,
eps:            d.EPS         !== “None” ? +d.EPS         : null,
dividendYield:  d.DividendYield !== “None” ? +d.DividendYield : null,
analystTarget:  d.AnalystTargetPrice !== “None” ? +d.AnalystTargetPrice : null,
beta:           d.Beta        !== “None” ? +d.Beta        : null,
high52:         d[“52WeekHigh”] !== “None” ? +d[“52WeekHigh”] : null,
low52:          d[“52WeekLow”]  !== “None” ? +d[“52WeekLow”]  : null,
mktCap:         d.MarketCapitalization !== “None” ? +d.MarketCapitalization : null,
revenueGrowth:  d.QuarterlyRevenueGrowthYOY !== “None” ? +(+d.QuarterlyRevenueGrowthYOY*100).toFixed(2) : null,
epsGrowth:      d.QuarterlyEarningsGrowthYOY !== “None” ? +(+d.QuarterlyEarningsGrowthYOY*100).toFixed(2) : null,
source:         “alphavantage”,
};
_setCache(`av_overview_${ticker}`, result);
return result;
}catch(e){ return null; }
}

// ── Master data fetcher: combines all sources ─────────────────────────────────
// Returns a fully verified RealData object with locked prices and fundamentals
async function fetchRealData(ticker) {
const cached = _getCached(`rd_${ticker}`);
if(cached) return cached;

// Run all fetches in parallel
const [yahooQuote, finnhubQuote, finnhubMetrics, finnhubRecs, finnhubEarnings, finnhubProfile] =
await Promise.all([
...
// Price: Finnhub is most accurate (real-time), Yahoo is fallback
const priceSource = finnhubQuote || yahooQuote;
if(!priceSource) return null; // no data available at all

// Fundamentals: Finnhub metrics first, Alpha Vantage as fallback
// (Alpha Vantage only called if Finnhub key missing to conserve daily limit)
let fundamentals = finnhubMetrics;
let overview     = null;
if(!fundamentals && ALPHAVANTAGE_KEY){
overview     = await fetchAVOverview(ticker);
fundamentals = overview ? {
peRatio:       overview.peRatio,
eps:           overview.eps,
revenueGrowth: overview.revenueGrowth,
epsGrowth:     overview.epsGrowth,
...
// Analyst target price — from AV overview if available
const analystTarget = overview?.analystTarget || null;

const result = {
ticker,
// LOCKED PRICE — AI must not override these
price:         priceSource.price,
change:        priceSource.change,
changePct:     priceSource.changePct,
high:          finnhubQuote?.high    || yahooQuote?.high52,
low:           finnhubQuote?.low     || yahooQuote?.low52,
high52:        fundamentals?.high52  || yahooQuote?.high52,
low52:         fundamentals?.low52   || yahooQuote?.low52,
volume:        yahooQuote?.volume,
mktCap:        finnhubProfile?.mktCap || yahooQuote?.mktCap,
...
fundamentalSource: fundamentals?.source || “unavailable”,
// ANALYST DATA
analystTarget,
analystRecs:   finnhubRecs,
analystConsensus: finnhubRecs?.consensus || null,
analystBullishPct: finnhubRecs?.bullishPct || null,
// EARNINGS
nextEarnings:  finnhubEarnings?.date     || null,
earningsEPS:   finnhubEarnings?.epsEstimate || null,
// PROFILE
name:          finnhubProfile?.name,
sector:        finnhubProfile?.sector,
// QUANT SCORE (computed below)
quantScore:    null,
quantGrade:    null,
};

// ── Proprietary Quantitative Scoring Model ────────────────────────────────
// Each dimension scored 0-25, total 0-100
// Based on real verified numbers only — no AI input
let score = 0;
let scoredDimensions = 0;

// DIMENSION 1: Momentum (0-25)
// Price vs 52-week range position + day change
if(result.high52 && result.low52 && result.price){
const range   = result.high52 - result.low52;
const pos     = range > 0 ? (result.price - result.low52) / range : 0.5;
const dayMom  = Math.min(Math.max(result.changePct / 5, -1), 1); // normalize -5%/+5% to -1/+1
const momScore = ((pos * 0.7) + ((dayMom + 1) / 2 * 0.3)) * 25;
score += momScore;
scoredDimensions++;
}

// DIMENSION 2: Fundamental Quality (0-25)
// Revenue growth, EPS growth, gross margin
if(result.revenueGrowth !== null || result.epsGrowth !== null){
let fundScore = 12.5; // neutral baseline
if(result.revenueGrowth !== null){
fundScore += Math.min(Math.max(result.revenueGrowth / 4, -6), 6); // ±6 pts for growth
}
if(result.epsGrowth !== null){
fundScore += Math.min(Math.max(result.epsGrowth / 4, -6), 6);
}
if(result.grossMargin !== null){
fundScore += (result.grossMargin / 100) * 5; // up to 5pts for high margins
}
score += Math.min(Math.max(fundScore, 0), 25);
scoredDimensions++;
}

// DIMENSION 3: Valuation (0-25)
// P/E relative to growth (PEG-like), P/B
if(result.peRatio !== null){
let valScore = 12.5;
// Penalize extreme overvaluation, reward reasonable valuation
if(result.peRatio > 0 && result.peRatio < 15)       valScore = 22;
else if(result.peRatio >= 15 && result.peRatio < 25) valScore = 18;
else if(result.peRatio >= 25 && result.peRatio < 40) valScore = 14;
else if(result.peRatio >= 40 && result.peRatio < 60) valScore = 10;
else if(result.peRatio >= 60)                        valScore = 6;
else if(result.peRatio < 0)                          valScore = 4; // negative earnings
// Boost if growing fast (PEG adjustment)
if(result.epsGrowth > 30 && result.peRatio < 50)     valScore += 3;
score += Math.min(Math.max(valScore, 0), 25);
scoredDimensions++;
}

// DIMENSION 4: Analyst Conviction (0-25)
// Based on verified analyst consensus from Finnhub
if(result.analystBullishPct !== null){
score += (result.analystBullishPct / 100) * 25;
scoredDimensions++;
}

// Normalize if not all dimensions available
const quantScore = scoredDimensions > 0
? Math.round((score / (scoredDimensions * 25)) * 100)
: null;

result.quantScore = quantScore;
result.quantGrade = quantScore >= 80 ? “A” :
quantScore >= 65 ? “B” :
quantScore >= 50 ? “C” :
quantScore >= 35 ? “D” : “F”;

// Implied upside from analyst target
if(analystTarget && result.price){
result.impliedUpside = +(((analystTarget - result.price) / result.price) * 100).toFixed(1);
}

_setCache(`rd_${ticker}`, result);
return result;
}

// ── Build locked data context string for AI prompts ───────────────────────────
// This string is prepended to every AI analysis prompt.
// The AI is FORBIDDEN from changing prices or fundamentals.
function buildLockedContext(rd) {
if(!rd) return “”;
const fmt = (v, suffix=””) => v !== null && v !== undefined ? `${v}${suffix}` : “N/A”;
const fmtPct = (v) => v !== null && v !== undefined ? `${v > 0 ? "+" : ""}${v}%` : “N/A”;
const fmtPrice = (v) => v !== null && v !== undefined ? `$${v}` : “N/A”;

return `
VERIFIED REAL-TIME DATA — YOU MUST USE THESE EXACT NUMBERS. DO NOT CHANGE OR ESTIMATE ANY OF THESE VALUES:

- Current Price: ${fmtPrice(rd.price)} (source: ${rd.priceSource}) — THIS IS THE REAL CURRENT PRICE
- Day Change: ${fmtPct(rd.changePct)} (${fmtPrice(rd.change)})
- 52-Week Range: ${fmtPrice(rd.low52)} — ${fmtPrice(rd.high52)}
- Market Cap: ${rd.mktCap ? “$”+Math.round(rd.mktCap/1e9)+“B” : “N/A”}
- P/E Ratio: ${fmt(rd.peRatio)} (source: ${rd.fundamentalSource})
- EPS: ${fmtPrice(rd.eps)}
- Revenue Growth (YoY): ${fmtPct(rd.revenueGrowth)}
- EPS Growth (YoY): ${fmtPct(rd.epsGrowth)}
- Gross Margin: ${fmtPct(rd.grossMargin)}
- ROE: ${fmtPct(rd.roe)}
- Beta: ${fmt(rd.beta)}
- Analyst Consensus: ${fmt(rd.analystConsensus)} (${fmt(rd.analystBullishPct)}% bullish, ${fmt(rd.analystRecs?.total)} analysts)
- Analyst Price Target: ${fmtPrice(rd.analystTarget)} (implied upside: ${fmtPct(rd.impliedUpside)})
- Next Earnings: ${fmt(rd.nextEarnings)}
- Quant Score: ${fmt(rd.quantScore)}/100 (Grade: ${fmt(rd.quantGrade)})

YOUR ROLE: Using the above VERIFIED data as your foundation, provide the investment thesis, catalysts, risks, and rating. The price shown above is correct — do not second-guess it.`;
}
...
const gradeColor = result.quantGrade===“A”?GREEN:result.quantGrade===“B”?”#7ee8a2”:result.quantGrade===“C”?GOLD:result.quantGrade===“D”?ORANGE:RED;
return(
...
{result.quantGrade&&(
<div style={{background:gradeColor+“22”,border:`1px solid ${gradeColor}55`,borderRadius:4,padding:“3px 10px”,display:“flex”,alignItems:“center”,gap:6}}>
<span style={{fontFamily:”‘DM Sans’,sans-serif”,fontSize:10,fontWeight:700,color:MUTED,letterSpacing:0.5}}>QUANT</span>
<span style={{fontFamily:”‘Bebas Neue’,cursive”,fontSize:16,color:gradeColor,letterSpacing:1}}>{result.quantGrade}</span>
{result.quantScore&&<span style={{fontFamily:”‘DM Sans’,sans-serif”,fontSize:11,color:gradeColor}}>{result.quantScore}/100</span>}
</div>
)}
<div style={{background:`${BLUE}11`,border:`1px solid ${BLUE}33`,borderRadius:4,padding:“3px 8px”}}>
<span style={{fontFamily:”‘DM Sans’,sans-serif”,fontSize:9,fontWeight:700,color:BLUE,letterSpacing:0.5}}>VERIFIED DATA</span>
</div>
</div>
<div style={{fontFamily:”‘Bebas Neue’,cursive”,fontSize:26,color:c.color,marginTop:4,letterSpacing:2}}>{result.confidence}% CONFIDENCE</div>
</div>
<div style={{display:“flex”,gap:8,flexWrap:“wrap”}}>
{result.price&&<InfoBox label="Live Price" value={result.price}/>}
{result.priceTarget&&<InfoBox label="Analyst Target" value={result.priceTarget} color={GREEN}/>}
{result.upside&&<InfoBox label="Implied Upside" value={result.upside} color={GREEN}/>}
{result.analystConsensus&&<InfoBox label="Analyst Consensus" value={result.analystConsensus}/>}
</div>
</div>
...
```
  {/* Quant Score Bar */}
  {result.quantScore&&(
    <div style={{background:DIM,borderRadius:6,padding:"10px 14px",borderLeft:`2px solid ${gradeColor}`}}>
      <div style={{display:"flex",justifyContent:"space-between",marginBottom:6}}>
        <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:11,fontWeight:700,color:MUTED,letterSpacing:0.5,textTransform:"uppercase"}}>Quantitative Score</span>
        <span style={{fontFamily:"'Bebas Neue',cursive",fontSize:16,color:gradeColor}}>{result.quantScore}/100 — Grade {result.quantGrade}</span>
      </div>
      <div style={{height:5,background:BORDER,borderRadius:3,overflow:"hidden"}}>
        <div style={{height:"100%",width:`${result.quantScore}%`,background:`linear-gradient(90deg,${gradeColor}88,${gradeColor})`,borderRadius:3,transition:"width 0.8s ease"}}/>
      </div>
      <div style={{display:"flex",justifyContent:"space-between",marginTop:4}}>
        {["Momentum","Fundamentals","Valuation","Analyst Conviction"].map(d=>(
          <span key={d} style={fm(MUTED,9,{letterSpacing:0.3})}>{d}</span>
        ))}
      </div>
    </div>
  )}
  ...
  {/* Score bar */}
  <ScoreBar label="AI Confidence" value={result.confidence||0} color={c.color}/>

  {/* Key metrics */}
  <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
    {result.eps&&<InfoBox label="EPS" value={result.eps}/>}
    {result.revenue&&<InfoBox label="Revenue" value={result.revenue}/>}
    {result.peRatio&&<InfoBox label="P/E Ratio" value={result.peRatio}/>}
...
setState(“loading”);
try{
// Fetch verified real data first — AI cannot override price or fundamentals
const rd = await fetchRealData(stock.ticker);
const locked = buildLockedContext(rd);
const r = await callAI(STOCK_SYSTEM,
`${locked}

Analyze ${stock.ticker} (${stock.name}) in ${stock.sector}. Today: ${new Date().toLocaleDateString(“en-US”,{year:“numeric”,month:“long”,day:“numeric”})}. ${stock.whyThisWeek||stock.whyToday||””}

IMPORTANT: Use the exact price $${rd?.price||“unknown”} in your response. Do not invent a different price.`, ); // Override AI price with verified real price if(rd?.price) r.price = `$${rd.price}`; if(rd?.quantScore) r.quantScore = rd.quantScore; if(rd?.quantGrade) r.quantGrade = rd.quantGrade; if(rd?.analystTarget) r.priceTarget = `$${rd.analystTarget}`; if(rd?.impliedUpside) r.upside = `+${rd.impliedUpside}%`; if(rd?.peRatio) r.peRatio = `${rd.peRatio}`;
if(rd?.analystConsensus) r.analystConsensus = rd.analystConsensus;
setResult(r);
setState(“done”);
...
setState(“loading”);
try{
const rd = await fetchRealData(gem.ticker);
const locked = buildLockedContext(rd);
const r = await callAI(GEM_SYSTEM,`${locked}

Analyze hidden gem ${gem.ticker} (${gem.name}) in ${gem.sector||“Unknown”}. Thesis: ${gem.why}. Today: ${new Date().toLocaleDateString(“en-US”,{year:“numeric”,month:“long”,day:“numeric”})}.

IMPORTANT: Use the exact price $${rd?.price||“unknown”}. Do not invent a different price.`); if(rd?.price) r.price = `$${rd.price}`; if(rd?.analystTarget) r.priceTarget = `$${rd.analystTarget}`; if(rd?.impliedUpside) r.upside = `+${rd.impliedUpside}%`; if(rd?.peRatio) r.peRatio = `${rd.peRatio}`;
if(rd?.quantScore) r.quantScore = rd.quantScore;
...
try{
const rd = await fetchRealData(selectedTicker);
const locked = buildLockedContext(rd);
const r = await callAI(OPTIONS_SYSTEM,
`${locked}

Generate 3 options setups for ${selectedTicker} (${selectedStock.name}). Today: ${new Date().toLocaleDateString(“en-US”,{year:“numeric”,month:“long”,day:“numeric”})}. Stock price is EXACTLY $${rd?.price||“unknown”} — use this for all strike calculations. Next earnings: ${rd?.nextEarnings||“search for it”}. Beta: ${rd?.beta||“N/A”}.`); if(rd?.price) r.stockPrice =`$${rd.price}`;
setResult(r); setState(“done”);
}catch{ setState(“error”); }
};
...
const rd2 = await fetchRealData(sym);
const locked2 = buildLockedContext(rd2);
const r = await callAI(OPTIONS_SYSTEM,
`${locked2}

Generate 3 options setups for ${sym}. Today: ${new Date().toLocaleDateString(“en-US”,{year:“numeric”,month:“long”,day:“numeric”})}. Stock price is EXACTLY $${rd2?.price||“unknown”} — use this for all strike calculations. Next earnings: ${rd2?.nextEarnings||“search for it”}.`); if(rd2?.price) r.stockPrice =`$${rd2.price}`;
...
try{
const rd = await fetchRealData(t);
const locked = buildLockedContext(rd);
const r = await callAI(SCREENER_SYSTEM,
`${locked}

Screen this stock: ${t}. Today: ${new Date().toLocaleDateString(“en-US”,{year:“numeric”,month:“long”,day:“numeric”})}. Required filters: confidence>=${filters.minConf}%, verdict=”${filters.verdict===“All”?“any”:filters.verdict}”, momentum=”${filters.momentum===“All”?“any”:filters.momentum}”, upside>=${filters.minUpside}%.

IMPORTANT: Use the exact verified price $${rd?.price||“unknown”} in your response. The quant score for this stock is ${rd?.quantScore||“N/A”}/100. If stock does not meet ALL criteria set passesScreen to false.`); if(rd?.price) r.price =`$${rd.price}`; if(rd?.analystTarget) r.priceTarget = `$${rd.analystTarget}`; if(rd?.impliedUpside) r.upside = `+${rd.impliedUpside}%`; if(rd?.peRatio) r.peRatio = `${rd.peRatio}`; if(rd?.revenueGrowth) r.revenueGrowth = `+${rd.revenueGrowth}% YoY`; if(rd?.epsGrowth) r.earningsGrowth = `+${rd.epsGrowth}% YoY`;
if(rd?.analystConsensus) r.analystConsensus = rd.analystConsensus;
if(rd?.quantScore){ r.quantScore = rd.quantScore; r.quantGrade = rd.quantGrade; }
setScanned(s=>s+1);
...
const rd = await fetchRealData(ticker);
const locked = buildLockedContext(rd);
const r = await callAI(STOCK_SYSTEM,
`${locked}

Analyze ${ticker}. User holds ${h?.shares||””} shares at avg cost $${h?.avgCost||””}. Today: ${new Date().toLocaleDateString(“en-US”,{year:“numeric”,month:“long”,day:“numeric”})}.

IMPORTANT: The verified current price is $${rd?.price||lp?.price?.toFixed(2)||“unknown”}. Use this exact price.`); if(rd?.price) r.price =`$${rd.price}`; if(rd?.analystTarget) r.priceTarget = `$${rd.analystTarget}`; if(rd?.impliedUpside) r.upside = `+${rd.impliedUpside}%`; if(rd?.peRatio) r.peRatio = `${rd.peRatio}`;
if(rd?.quantScore){ r.quantScore = rd.quantScore; r.quantGrade = rd.quantGrade; }
...
// ── Fetch function registry — populated after functions are defined ──────
// This ref holds the latest versions of all fetch functions, solving the
// forward-reference problem where autoFetchAll was defined before the
// fetch functions it calls existed in the closure.
const fetchRegistry = useRef({});
...
const reg = fetchRegistry.current;

```
// Only fetch if cache is stale
const fetchIfStale = async (cacheKey, fnName, maxAgeMs=30*60*1000) => {
  try {
    ...
    const fn = reg[fnName];
    if(fn) await fn();
  } catch(e){ console.warn("autoFetch error:", fnName, e.message); }
};

if(open||extended){
  // Wave 1 — watchlist + trending (30 min cache)
  await fetchIfStale(CK.watchlist,      "fetchWatchlist",      30*60*1000);
  await new Promise(r=>setTimeout(r,1500));
  await fetchIfStale(CK.trending,       "fetchTrending",       30*60*1000);
  await new Promise(r=>setTimeout(r,1500));
  // Wave 2 — gems + IPOs (1 hour cache)
...
if(open){
  await new Promise(r=>setTimeout(r,1500));
  // Wave 4 — day trading (15 min cache, most time-sensitive)
...
if(open||extended){
  await new Promise(r=>setTimeout(r,1500));
  // Wave 5 — extended hours data (20 min cache)
...
```
// Step 2: Start staggered auto-fetch after registry is populated
// 100ms ensures the fetchRegistry useEffect (which runs every render) fires first
const t = setTimeout(()=>autoFetchAll(), 3000);
...
try{
// Try to fetch real data — ticker might need to be identified first by AI if it’s a company name
const tickerGuess = input.toUpperCase().replace(/[^A-Z]/g,””);
const rd = tickerGuess.length <= 5 ? await fetchRealData(tickerGuess) : null;
const locked = rd ? buildLockedContext(rd) : “”;
const r = await callAI(STOCK_SYSTEM,
`${locked}

The user searched for: “${input}”. This could be a ticker OR company name. Identify the correct US stock ticker and analyze it. Today: ${new Date().toLocaleDateString(“en-US”,{year:“numeric”,month:“long”,day:“numeric”})}.${rd?.price ? `

IMPORTANT: The verified current price is $${rd.price}. Use this exact price.` : ""}`
);
if(rd?.price) r.price = `$${rd.price}`;
if(rd?.analystTarget) r.priceTarget = `$${rd.analystTarget}`;
if(rd?.impliedUpside) r.upside = `+${rd.impliedUpside}%`;
if(rd?.peRatio) r.peRatio = `${rd.peRatio}`;
if(rd?.quantScore){ r.quantScore = rd.quantScore; r.quantGrade = rd.quantGrade; }
...
// ── Register all fetch functions into the ref so autoFetchAll can call them ──
// This must come AFTER all fetch functions are defined.
useEffect(()=>{
fetchRegistry.current = {
fetchWatchlist,
fetchTrending,
fetchSmActivity,
fetchDynamicOptions,
...
// Update registry whenever any fetch function identity changes
// eslint-disable-next-line react-hooks/exhaustive-deps
});
